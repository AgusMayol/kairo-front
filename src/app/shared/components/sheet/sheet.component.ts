...
@Component({
  ...
  <div class="bg-sidebar h-full">
    ...
    @if (config.zTitle || config.zDescription) {
      <header data-slot="sheet-header" class="flex flex-col gap-1.5 p-4 bg-sidebar">
        @if (config.zTitle) {
    ...
    <main class="flex flex-col space-y-4 w-full h-full pb-10 bg-sidebar">
      <ng-template cdkPortalOutlet></ng-template>

      @if (isStringContent) {
        <div data-testid="z-content" data-slot="sheet-content" class="bg-sidebar" [innerHTML]="config.zContent"></div>
      }
    </main>

    @if (!config.zHideFooter) {
      <footer data-slot="sheet-footer" class="mt-auto flex flex-col gap-2 p-4 bg-sidebar">
